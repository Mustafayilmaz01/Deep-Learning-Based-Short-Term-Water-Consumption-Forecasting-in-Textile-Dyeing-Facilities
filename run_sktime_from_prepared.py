...
Split: Train=108 | Val=24 | Test=12  |  Window=12
"""
...
# ── Çevre Değişkenleri ve Log Seviyeleri ─────────────────────────────────────
os.environ.update({
...
# ── Reproducibility (Tekrar Edilebilirlik) ───────────────────────────────────
RANDOM_SEED = 1
...
# Dizinleri oluştur
sub_dirs = [
    "", "hyperparameter_plots", "model_comparisons", "time_series_plots",
...
def norm3d_fit(Xtr, Xte):
    _, c, _ = Xtr.shape
    Xtr_n = np.zeros_like(Xtr)
    Xte_n = np.zeros_like(Xte)
    scs = []
...
def norm3d_apply(X, scs):
    ...
    for i, sc in enumerate(scs): 
        Xo[:, i, :] = sc.transform(X[:, i, :])
    return Xo

def norm3d(Xtr, Xte):
    Xtr_n, Xte_n, _ = norm3d_fit(Xtr, Xte)
    return Xtr_n, Xte_n
...
def split_from_meta(n, meta):
    ...
    if ntr is None or nva is None: 
        return time_split(n, TRAIN_RATIO, VAL_RATIO)
...
def _inner_keras(m):
    for a in ("model_", "network_", "clf_"):
        ...
        for s in ("model_", "model", "network_"):
            ...
            if km is not None and hasattr(km, "fit") and hasattr(km, "history"): 
                return km
        if hasattr(obj, "fit") and hasattr(obj, "history"): 
            return obj
    return None
...
def _get_history(m):
    ...
    if h is not None: 
        cands.append(h.history if hasattr(h, "history") else h)
    ...
    for c in cands:
        if isinstance(c, dict) and c.get("loss"): 
            return c
    return {}
    ...
    if val_data is not None:
        ...
        if km is not None:
            ...
            try:
                ...
                callbacks = [cb for cb in [_make_es("val_loss"), _make_reduce_lr()] if cb]
                history = new_km.fit(
...
def plot_full_series(y_tr, y_va, y_te, y_pred_val, y_pred_test, path, name,
    ...
    xtr = np.arange(0, ntr)
    xva = np.arange(ntr, ntr + nva)
    xte = np.arange(ntr + nva, ntot)
...
def plot_forecast(y_true, y_pred, path, name, ts=None, ci_lo=None, ci_hi=None, label="Test"):
    ...
    if ci_lo is not None: 
        ax.fill_between(x, ci_lo, ci_hi, alpha=0.15, color=col, label="95% CI")
...
def plot_parity(y_true, y_pred, path, name):
    ...
    ax.set_xlabel("Actual W-Water (m³)")
    ax.set_ylabel("Predicted W-Water (m³)")
    ax.set_title(f"Parity – {nm}")
    ax.grid(True, alpha=0.3)
    save_fig(path)
...
def plot_all_subplots(y_true, preds, path, ts=None, ci_dict=None):
    ...
    for i, (nm, yp) in enumerate(preds.items()):
        ...
        ax.set_ylabel("W-Water (m³)")
        ax.set_title(disp(nm))
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.3)
        
        if ts is not None and len(ts) == n:
            step = max(1, n // 5)
            idx2 = list(range(0, n, step))
            ax.set_xticks(idx2)
...
def plot_error_boxplot(preds, y_true, path):
    ...
    for patch, nm in zip(bp["boxes"], preds): 
        patch.set_facecolor(COLORS.get(nm.lower(), "#1f77b4"))
        patch.set_alpha(0.6)
        
    ax.set_ylabel("Absolute Error (m³)")
    ax.set_title("Absolute Error Distribution – All Models")
...
def plot_parity_grid(y_true, preds, path, ncols=3):
    ...
    gmin = min(v.min() for v in all_v)
    gmax = max(v.max() for v in all_v)
    pad = (gmax - gmin) * 0.05
    lims = [gmin - pad, gmax + pad]
...
def plot_hp_grid(tuning_res, name, path):
    PLABELS = {
        "n_epochs": "Number of Epochs", "batch_size": "Batch Size", "lstm_size": "LSTM Units",
        "num_kernels": "Num Kernels", "n_filters": "Num Filters", "depth": "Network Depth"
    }
    np_ = len(tuning_res)
    if np_ == 0: 
        return
    ...
    for i, (pn, data) in enumerate(tuning_res.items()):
        ...
        ax1.set_xticks(xp)
        ax1.set_xticklabels([str(v) for v in vals], fontsize=8)
    ...
    for j in range(np_, len(axes.flatten())): 
...
def plot_comp_time(train_t, path, inf_t=None):
    models_ = list(train_t.keys())
    labels = [disp(m) for m in models_]
    ...
    for bar, t in zip(bars, tr):
        if t > 0: 
            ax.text(bar.get_x() + bar.get_width() / 2., bar.get_height() + max(tr) * 0.01, f"{t:.1f}s", ha="center", va="bottom", fontsize=7.5)
    ...
    if has_inf:
        ...
        for bar, t in zip(bi, iv):
            if t > 0: 
                ax.text(bar.get_x() + bar.get_width() / 2., bar.get_height() + max(tr) * 0.01, f"{t:.3f}s", ha="center", va="bottom", fontsize=7.5)
...
def plot_seed_var(seed_res, path):
    ...
    for ax, met in zip(axes, mets):
        ...
        for xi, v, c in zip(x, raw, cols):
            if v: 
                ax.scatter(xi + rng.uniform(-0.13, 0.13, size=len(v)), v, s=36, color=c, edgecolors="black", linewidths=0.6, zorder=4)
...
def run_arima(ytr, yte):
    ...
    for t in range(len(yte)):
        try: 
            fc = ARIMA(hist, order=best_ord).fit().forecast(1)[0]
        except Exception: 
            fc = hist[-1]
        preds.append(fc)
        hist.append(yte[t])
...
def tune_model(cls, base_kw, grid, Xtr, ytr_sc, yscaler, name, Xva, yva_sc, Xtrva, ytrva_sc):
    ...
    for pn, vals in grid.items():
        ...
        for v in vals:
            kw = {**base_kw, pn: v}
            if "callbacks" in kw: 
                kw["callbacks"] = [cb for cb in [_make_es()] if cb]
            ...
            try:
                ...
                if rmse < best_score:
                    ...
                    if "callbacks" in kw2: 
                        kw2["callbacks"] = [cb for cb in [_make_es()] if cb]
                        
                    set_all_seeds(RANDOM_SEED)
                    t0 = time.time()
                    bc = cls(**{**kw2, "random_state": RANDOM_SEED})
                    h_f = fit_capture(bc, Xtrva, ytrva_sc, val_data=None)
                    total_t += time.time() - t0
                    
                    hp = h_t if (h_t and h_t.get("val_loss")) else h_f
                    if hp: 
                        bc._captured_history = hp
                    best_model = bc
    ...
    for a in ("model_", "model", "network_", "clf_"):
        ...
        for s in ("model_", "model", "network_"):
            ...
            if k and hasattr(k, "count_params"):
                try: 
                    return int(k.count_params())
                except Exception: 
                    pass
        if hasattr(obj, "count_params"):
            try: 
                return int(obj.count_params())
            except Exception: 
                pass
    return None
...
def _import_inception():
    for cn in ("InceptionTimeRegressor", "InceptionTime"):
        try: 
            return getattr(__import__("sktime.regression.deep_learning", fromlist=[cn]), cn)
        except ImportError: 
            pass
...
def _save_model_results(name, ytr, yva, yte, yp_val, yp_test,
    ...
    all_predictions[name] = yp_test
    all_ci[name] = (ci_lo, ci_hi)
    
    if yp_val is not None:
        all_val_predictions[name] = yp_val
        all_ci_val[name] = (ci_lo_v, ci_hi_v)
    ...
    val_results.append({
        "model": name, "MAE": mae_v, "MSE": mse_v, "RMSE": rmse_v, "MAPE(%)": mape_v,
        "MASE": mase_v, "RMSSE": rmsse_v, "Training_Time_s": train_t, "Inference_Time_s": inf_t
    })
    results.append({
        "model": name, "MAE": mae, "MSE": mse, "RMSE": rmse, "MAPE(%)": mape,
    ...
    if yp_val is not None:
        ...
        pd.DataFrame({
            "y_true": yva, "y_pred": yp_val, "residual": yva - yp_val,
    ...
    pd.DataFrame({
        "y_true": yte, "y_pred": yp_test, "residual": yte - yp_test,
...
try: 
    ts = pd.to_datetime(pd.read_csv(pj(PREP_DIR, "timestamps.csv"))["target_month"]).values
except Exception: 
    ts = None

X3d = np.transpose(X, (0, 2, 1))
n = X3d.shape[0]
assert len(y) == n
...
def dr(k): 
    return f"{meta.get(k, ['?', '?'])[0]} – {meta.get(k, ['?', '?'])[1]}"
...
models = {
    ...
    "LSTMFCNRegressor": {
        ...
        "base_kwargs": dict(
            n_epochs=ES_MAX_EPOCHS, batch_size=8, lstm_size=8,
            dropout=0.3, random_state=RANDOM_SEED, verbose=False, callbacks=[_make_es()],
        ),
    },
    "InceptionTime": {
        "class": InceptionTimeRegressor,
        "base_kwargs": dict(
            n_epochs=ES_MAX_EPOCHS, batch_size=16, n_filters=32, depth=6,
...
section("TRAINING & HYPERPARAMETER SELECTION  [Multi-seed: seeds 1-60]")
results, val_results, all_predictions, all_val_predictions = [], [], {}, {}
...
for name, cfg in models.items():
    ...
    train_times[name] = tr_t
    best_params_log[name] = best_p
    all_tune_res[name] = tune_res
    models[name]["best_params"] = best_p
    ...
    if h: 
        ...
        plot_epoch_loss(h, name, tc)

    pc = count_params(model)
    if pc: 
        param_counts[name] = pc
        print(f"  Trainable params: {pc:,}")
    ...
    for seed in SEEDS:
        try:
            ...
            # 1. Validation İçin Model Eğitimi (Sadece Train verisi)
            ms_val = cls(**{**bkw2_copy, "random_state": seed})
            ms_val.fit(Xtr_n, ytr_sc)
            yp_va_sc_ref = ms_val.predict(Xva_n)
            
            if not np.isnan(yp_va_sc_ref).any():
                yp_v_s = ysc.inverse_transform(yp_va_sc_ref.reshape(-1, 1)).ravel()
                val_preds_list.append(yp_v_s)

            # 2. Test İçin Model Eğitimi (Train + Validation verisi)
            ms_test = cls(**{**bkw2_copy, "random_state": seed})
            ms_test.fit(Xtrva_n, ytrva_sc)
            yp_te_sc = ms_test.predict(Xte_n)
            
            if np.isnan(yp_te_sc).any():
                print(f"    seed={seed}: NaN test – atlandı")
                continue
                
            yp_t_s = ysc.inverse_transform(yp_te_sc.reshape(-1, 1)).ravel()
            test_preds_list.append(yp_t_s)

            # Metriklerin Toplanması (Test Metrikleri)
    ...
    if not test_preds_list:
        print(f"  [WARN] {disp(name)}: hiçbir seed başarılı olmadı, atlandı.")
        continue

    # Ensemble-Ortalama Tahminler
    ...
    # Ensemble Test Metrikleri Hesaplaması
    mae_avg, mse_avg, rmse_avg, mape_avg, mase_avg, rmsse_avg = compute_metrics(yte, yp_test_avg, ytrva)
    te_metrics = {
        "MAE": mae_avg, "MSE": mse_avg, "RMSE": rmse_avg,
        "MAPE(%)": mape_avg, "MASE": mase_avg, "RMSSE": rmsse_avg,
    }
    
    sr_te = seed_results[name]
    def _nanmean_sr(lst): return float(np.nanmean(lst)) if lst else np.nan
    print(f"  {disp(name)} – PER-SEED MEAN: "
          f"RMSE={_nanmean_sr(sr_te['RMSE']):.2f}  MAE={_nanmean_sr(sr_te['MAE']):.2f}  "
          f"MAPE={_nanmean_sr(sr_te['MAPE(%)']):.4f}% "
          f"  [ENSEMBLE-AVG → RMSE={rmse_avg:.2f}  MAE={mae_avg:.2f}  MAPE={mape_avg:.4f}%]")

    # Ensemble Validation Metrikleri Hesaplaması
    val_metrics = None
    if yp_val_avg is not None:
        vm_avg, vmse_avg, vr_avg, vmp_avg, vmas_avg, vrms_avg = compute_metrics(yva, yp_val_avg, ytr)
        val_metrics = {
            "MAE": vm_avg, "MSE": vmse_avg, "RMSE": vr_avg,
            "MAPE(%)": vmp_avg, "MASE": vmas_avg, "RMSSE": vrms_avg,
        }
    ...
    print(f"\n  {disp(name)} done  (ortalama {len(test_preds_list)} seed).")

if all_histories: 
    plot_training_curves_combined(all_histories, tc)

section("BASELINE MODELS  [RF & XGBoost multi-seed, istatistiksel deterministik]")
det_bl_fns = {
...
for bname, (test_fn, val_fn) in det_bl_fns.items():
    ...
    try:
        ...
        train_times[bname] = bt
        infer_times[bname] = 0.0
...
for bname, fns in stoch_bl.items():
    ...
    for seed in SEEDS:
        try:
            ...
            if not np.isnan(yp_t_s).any():
                ...
                bl_seed_mae.append(m_)
                bl_seed_mse.append(mse_)
                bl_seed_rmse.append(r_)
                bl_seed_mape.append(mp_)
                bl_seed_mase.append(mas_)
                bl_seed_rmsse.append(rms_)
            ...
            if not np.isnan(yp_v_s).any():
                ...
                bl_val_mae.append(vm_)
                bl_val_mse.append(vmse_)
                bl_val_rmse.append(vr_)
                bl_val_mape.append(vmp_)
                bl_val_mase.append(vmas_)
                bl_val_rmsse.append(vrms_)
                
        except Exception as e:
            print(f"    seed={seed}: HATA – {e}")

    if not bl_preds_te:
        print(f"  [WARN] {disp(bname)}: hiçbir seed başarılı olmadı.")
        continue

    yp_t_avg = np.mean(bl_preds_te, axis=0)
    yp_v_avg = np.mean(bl_preds_va, axis=0) if bl_preds_va else None
    
    train_times[bname] = t_total
    infer_times[bname] = 0.0

    def _nm(lst): 
        return float(np.nanmean([v for v in lst if not np.isnan(v)])) if any(not np.isnan(v) for v in lst) else np.nan
        
    bl_te_metrics = {
        "MAE": _nm(bl_seed_mae), "MSE": _nm(bl_seed_mse), "RMSE": _nm(bl_seed_rmse),
        "MAPE(%)": _nm(bl_seed_mape), "MASE": _nm(bl_seed_mase), "RMSSE": _nm(bl_seed_rmsse),
    }
    
    bl_val_metrics = {
        "MAE": _nm(bl_val_mae), "MSE": _nm(bl_val_mse), "RMSE": _nm(bl_val_rmse),
        "MAPE(%)": _nm(bl_val_mape), "MASE": _nm(bl_val_mase), "RMSSE": _nm(bl_val_rmsse),
    ...
    print(f"  {disp(bname)} done  (ortalama {len(bl_preds_te)} seed).")
...
for mn, ep in es_log.items(): 
    print(f"  Early stopping – {disp(mn)}: {ep} epochs")
    
if es_log: 
    pd.DataFrame([{"model": disp(k), "epochs_trained": v} for k, v in es_log.items()]).to_csv(pj(tc, "early_stopping_log.csv"), index=False)
...
print("\n  Window size sweep (all DL models incl. ROCKET)...")
...
if dl_seed: 
    plot_seed_var(dl_seed, pj(cp, "multi_seed_variance.png"))
if train_times: 
    plot_comp_time(train_times, pj(cp, "computation_time_training.png"), inf_t=infer_times)
...
res_df.to_csv(pj(OUTPUT_DIR, "results_test.csv"), index=False)
print("results_test.csv saved")
val_df.to_csv(pj(OUTPUT_DIR, "results_validation.csv"), index=False)
print("results_validation.csv saved")
...
def fmt_t(t): 
    return "<0.001" if (not np.isnan(t) and t < 0.001) else (f"{t:.3f}" if not np.isnan(t) else "")

pd.DataFrame([{
    "model": disp(m), "type": MTYPE.get(m, "—"),
    "training_time_s": fmt_t(train_times.get(m, np.nan)),
    "inference_time_s": fmt_t(infer_times.get(m, np.nan))
} for m in sorted(set(list(train_times) + list(infer_times)))]).to_csv(pj(OUTPUT_DIR, "computation_times.csv"), index=False)
...
for nm, data in seed_results.items():
    ...
    mean_r, std_r = _smean(rmse_vals), _sstd(rmse_vals)
    mean_m, std_m = _smean(mae_vals), _sstd(mae_vals)
    mean_p, std_p = _smean(mape_vals), _sstd(mape_vals)
    mean_mas, std_mas = _smean(mase_vals), _sstd(mase_vals)
    mean_rms, std_rms = _smean(rmsse_vals), _sstd(rmsse_vals)
...
def _fmv(v): 
    return "N/A" if (isinstance(v, float) and np.isnan(v)) else f"{v:.3f}"
...
print("\n" + "=" * 90)
print("TEST PERFORMANCE (sorted by RMSE):")
print("=" * 90)
...
for nm, data in seed_results.items():
    ...
    def _col(key): 
        return [r.get(key, np.nan) for r in rows if not (isinstance(r.get(key, np.nan), float) and np.isnan(r.get(key, np.nan)))]
    ...
    def _fmt(lst): 
        return f"{np.mean(lst):>10.4f}" if lst else f"{'N/A':>10}"
